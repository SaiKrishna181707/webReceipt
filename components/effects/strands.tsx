...
/* ============================================================================
   STRANDS — vendored from React Bits (ts-tailwind), then adapted.

   Ribbons of light crossing a panel, tapering off at both edges. Used behind one
   mid-page section, never on a console: it is a signature, not a texture.

   Changes from upstream:
     - `'use client'`
     - phosphor/teal ramp instead of red/violet/cyan/yellow (see ./palette)
     - honours `prefers-reduced-motion`: renders one frame, then stops
     - the render-target and glass program are built lazily, only if `glass` is
       actually used. Upstream allocates both on mount whether or not the prop is
       set, which costs a full-size framebuffer for an effect nothing asked for.
   ========================================================================== */
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    /* The glass pass needs a full-size framebuffer and a second program. Almost
       nothing turns it on, so it is built on first use and then kept. */
    let renderTarget: InstanceType<typeof RenderTarget> | null = null
    let glassProgram: InstanceType<typeof Program> | null = null
    let glassMesh: InstanceType<typeof Mesh> | null = null
    ...
    // Declared after `draw` on purpose: it repaints when the loop is parked, and
    // the first call happens immediately below.
    const resize = () => {
    ...
    // The strands are already unmounted when they leave the viewport by
    // GatedEffect; this only covers the tab going to the background.
    ...
    const onMotionPref = () => {
      ...
      media.matches ? (stop(), draw(performance.now())) : start()
    }
