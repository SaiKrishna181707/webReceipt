...
import { EFFECT_BRIGHT, EFFECT_DEEP } from './palette'

/* ============================================================================
   LIGHT TUNNEL — vendored from React Bits (ts-tailwind), then adapted.

   A radial field of cables running to a vanishing point, with data pulses
   travelling along them. This is the environment's L2: one full-screen fragment
   shader, one context, held for the life of the page.

   Changes from upstream:
     - `'use client'`, which the App Router requires and upstream does not carry
     - phosphor-green defaults instead of violet (see ./palette)
     - honours `prefers-reduced-motion`: renders one frame, then stops
     - dpr capped at 1.5 rather than 2, matching the rest of the environment
   ========================================================================== */
...
const LightTunnel: React.FC<LightTunnelProps> = ({
  cableColor = EFFECT_BRIGHT,
  pulseColor = EFFECT_BRIGHT,
  tunnelColor = EFFECT_DEEP,
  tunnelOpacity = 0,
  speed = 0.1,
  ...
  // Read inside the loop rather than in the dependency list: re-running this
  // effect would tear down and rebuild the GL context just to change a boolean.
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uOpacity: { value: 1.0 },
        uCableColor: { value: new Float32Array(hexToRgb(EFFECT_BRIGHT)) },
        uPulseColor: { value: new Float32Array(hexToRgb(EFFECT_BRIGHT)) },
        uTunnelColor: { value: new Float32Array(hexToRgb(EFFECT_DEEP)) },
    ...
    const loop = (t: number) => {
      ...
      // Checked here rather than at start-up so flipping the OS setting mid-page
      // stops the tunnel on the next frame instead of at the next reload.
    ...
    // A reduced-motion user who turns the setting off gets the loop started by
    // the same media-query listener that set the flag, via this handler.
    const media = window.matchMedia('(prefers-reduced-motion: reduce)')
    const onMotionPref = () => {
      reducedRef.current = media.matches
      media.matches ? tryStop() : tryStart()
    }
  ...
  useEffect(() => {
    ...
    // Under reduced motion the loop is not running, so a prop change would
    // otherwise sit in the uniforms unseen until something else forced a draw.
    if (reduced) ctx.renderer.render({ scene: ctx.mesh })
  }, [
    cableColor,
    pulseColor,
    tunnelColor,
