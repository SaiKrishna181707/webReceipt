...
export function DealContractCard({ contract, onEvidence }: DealContractCardProps) {
  ...
  return (
    <section className="glass-card rounded-2xl border border-white/10 overflow-hidden">
      {/* Header */}
      <div className="px-6 py-5 border-b border-white/10 bg-gradient-to-r from-violet-500/[0.08] to-transparent">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider text-violet-400 font-bold mb-1">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs font-mono text-gray-500 hover:text-cyan-400 transition-colors break-all"
            >
              {contract.targetUrl}
            </a>
          </div>
          <button
            ...
            className="shrink-0 flex items-center gap-1.5 text-xs font-mono text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 rounded-lg px-3 py-2 hover:bg-emerald-500/20 transition-colors"
        ...
        <div className="mt-4 flex items-end gap-6 flex-wrap">
          ...
          <ChevronRight className="text-gray-600 mb-2" size={20} />
      ...
      <div className="px-6 py-5 border-t border-white/10 grid sm:grid-cols-2 gap-3">
        ...
        {offer.claims.length > 0 && (
          <div className="sm:col-span-2 flex flex-wrap gap-2 pt-1">
            {offer.claims.map((c) => (
              <span key={c} className="text-[11px] font-mono text-cyan-300 bg-cyan-500/10 border border-cyan-500/30 rounded-full px-2.5 py-1">
                {c}
              </span>
            ))}
          </div>
...
function Headline({ label, value, accent, field, has, onClick }: { label: string; value: string; accent?: boolean; field?: string; has?: boolean; onClick?: (f: string) => void }) {
  ...
  return (
    ...
    >
      <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-1 flex items-center gap-1">
        {label}
        {clickable && <span className="text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity">· evidence</span>}
      </div>
      <div className={`font-bold font-mono ${accent ? 'text-3xl text-white' : 'text-2xl text-gray-300'} ${clickable ? 'group-hover:text-emerald-300 transition-colors' : ''}`}>
        {value}
      </div>
    </button>
  )
...
function Row({ label, sub, value, indent, field, has, onClick }: { label: string; sub?: string; value: string; indent?: boolean; field?: string; has?: boolean; onClick?: (f: string) => void }) {
  ...
  return (
    <button
      disabled={!clickable}
      onClick={() => field && onClick?.(field)}
      className={`w-full flex items-center justify-between py-1.5 rounded-lg px-2 -mx-2 ${clickable ? 'group hover:bg-emerald-500/[0.06] cursor-pointer' : 'cursor-default'} ${indent ? 'pl-5' : ''}`}
    >
      <span className={`text-sm ${indent ? 'text-gray-500' : 'text-gray-300'} flex items-center gap-2`}>
        {label}
        {sub && <span className="text-[10px] font-mono text-gray-600">({sub})</span>}
        {clickable && <span className="text-[10px] font-mono text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity">evidence →</span>}
      </span>
...
function Term({ label, value, field, has, onClick }: { label: string; value: string; field?: string; has?: boolean; onClick?: (f: string) => void }) {
  ...
  return (
    <button
      disabled={!clickable}
      onClick={() => field && onClick?.(field)}
      className={`text-left bg-white/[0.03] border border-white/10 rounded-xl p-3 ${clickable ? 'group hover:border-emerald-500/40 cursor-pointer' : 'cursor-default'}`}
    >
      <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-1 flex items-center gap-1">
        {label}
        {clickable && <span className="text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity">· evidence</span>}
