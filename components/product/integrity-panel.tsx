...
const STATUS = {
  valid: { icon: ShieldCheck, color: 'emerald', label: 'Contract valid' },
    ...
    <section className="glass-card rounded-2xl border border-white/10 overflow-hidden">
      <div className={`px-6 py-4 border-b border-white/10 flex items-center justify-between ${integrity.status === 'invalid' ? 'bg-rose-500/[0.07]' : ''}`}>
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-xl border ${COLOR[meta.color]}`}>
            <Icon size={18} />
          </div>
          <div>
        ...
        <div className="text-right">
          <div className="text-2xl font-bold font-mono text-white">
            {integrity.passed}<span className="text-gray-600">/{integrity.total}</span>
          </div>
          <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500">checks passed</div>
        </div>
      </div>
...
function CheckRow({ check }: { check: IntegrityCheck }) {
  ...
  return (
    <li>
      ...
      >
        <span className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${check.pass ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
          {check.pass ? <Check size={12} /> : <X size={12} />}
        </span>
        <span className={`flex-1 text-sm ${check.pass ? 'text-gray-300' : 'text-rose-300 font-medium'}`}>{check.label}</span>
        {!check.pass && (
        ...
        {hasDetails && <ChevronDown size={14} className={`text-gray-600 transition-transform ${open ? 'rotate-180' : ''}`} />}
      </button>
      {open && hasDetails && (
        <div className="px-6 pb-3 -mt-1">
          <pre className="bg-black/50 border border-white/10 rounded-lg p-3 text-[11px] font-mono text-gray-400 overflow-x-auto whitespace-pre-wrap break-words">
