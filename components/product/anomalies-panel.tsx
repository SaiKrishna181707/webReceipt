  ...
  return (
    <section className="glass-card rounded-2xl border border-white/10 overflow-hidden">
      <div className="px-6 py-4 border-b border-white/10">
        <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 font-bold">Deal Anomalies</div>
        <h3 className="font-bold text-white">Observed, not adjudicated</h3>
      </div>
      {anomalies.length === 0 ? (
        <div className="px-6 py-8 text-center text-sm text-gray-500">No anomalies observed in this contract.</div>
      ) : (
            ...
            return (
              <li key={a.id} className="px-6 py-4 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-1 leading-relaxed">{a.details}</p>
                </div>
              </li>
            )
          })}
