...
function consume(req: Request) {
  const now = Date.now(); const map = rateMap(); const key = clientKey(req); const current = map.get(key)
  const bucket = !current || current.resetAt <= now ? { count: 0, resetAt: now + WINDOW_MS } : current
  if (bucket.count >= REQUESTS_PER_WINDOW) fail('Public scraping rate limit reached. Try again shortly.')
  bucket.count += 1; map.set(key, bucket)
  if (map.size > 2000) for (const [candidate, value] of map) { if (value.resetAt <= now) map.delete(candidate); if (map.size <= 1500) break }
}
export async function withPublicScrapeLimit<T>(req: Request, task: () => Promise<T>): Promise<T> {
  consume(req); const active = globalRef.__webreceiptPublicActive || 0
  if (active >= MAX_CONCURRENT) fail('Public scraping is busy. Try again shortly.')
  globalRef.__webreceiptPublicActive = active + 1
  try { return await task() } finally { globalRef.__webreceiptPublicActive = Math.max(0, (globalRef.__webreceiptPublicActive || 1) - 1) }
}
