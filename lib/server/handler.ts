...
function classify(error: unknown): { status: number; code: string; message: string } {
  ...
  if (/Public page fetch timed out/i.test(message)) return { status: 504, code: 'public_fetch_timeout', message }
  if (/Public page (?:fetch failed|returned HTTP|redirect|exceeded)/i.test(message)) return { status: 502, code: 'public_fetch_failed', message }
  if (/Unknown public-web mutation/i.test(message)) return { status: 400, code: 'invalid_mutation', message }
  if (/Promise Diff contracts must belong to the same target URL|Promise Diff rejected a contract with an invalid seal/i.test(message)) return { status: 400, code: 'invalid_diff', message }
  ...
  if (/Bright Data live mode is not configured|Bright Data mode requires/i.test(message)) return { status: 503, code: 'brightdata_not_configured', message }
...
export async function runSafely<T>(fn: () => Promise<T>) {
  ...
  catch (error) { const mapped = classify(error); return NextResponse.json({ error: mapped.message, code: mapped.code }, { status: mapped.status }) }
}
