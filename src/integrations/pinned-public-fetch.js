...
async function pinnedRequest(input, init = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    request = transport.request(target, {
      ...
      agent: false,
      // The URL hostname remains authoritative for Host and TLS SNI, while the
      // socket is pinned to the address that passed the public-network check.
...
// User-controlled public URLs never go through native fetch after a separate DNS
// check. The fixed Bright Data API endpoint keeps native fetch for its POST body.
