...
export default function ConsolePage() {
  ...
  const observe = () =>
    run('observe', async () => {
      ...
      setDiff(null)
      setPhase('observed')
      toast.success('Commerce journey observed — Deal Contract compiled')
    })
  ...
  const breakIt = () =>
    run('break', async () => {
      ...
      setResult(r)
      setPhase('broken')
      toast.error('Page semantics changed — the same selector now carries a different meaning')
    })
    ...
    <div className="mx-auto max-w-7xl space-y-7 px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <ActionButton
          ...
          hint="Compile the baseline Deal Contract"
          icon={Play}
          tone="phosphor"
          onClick={observe}
          busy={busy === 'observe'}
