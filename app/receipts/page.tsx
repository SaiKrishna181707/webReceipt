...
const STATUS_COLOR = { valid: 'text-lime-400', warning: 'text-amber-400', invalid: 'text-rose-400' } as const
    ...
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-10 sm:px-6 lg:px-8">
      <header className="flex items-end justify-between gap-4 flex-wrap">
        <div className="space-y-2">
          <div className="text-[11px] font-mono uppercase tracking-[0.2em] text-azure-400">Receipts</div>
      ...
      {receipts.length === 0 ? (
        <div className="glass-card rounded-[8px] border border-dashed border-white/15 p-12 text-center">
          <div className="w-14 h-14 rounded-[8px] bg-azure-500/10 border border-azure-500/30 text-azure-400 flex items-center justify-center mx-auto mb-4">
            <Receipt size={22} />
          </div>
          ...
          <Link
            href="/console"
            className="inline-flex items-center gap-2 rounded-[6px] bg-brick-600 hover:bg-brick-500 text-white text-sm font-medium px-5 py-3 transition-colors"
          >
            Open Console
          </Link>
        </div>
            ...
            {receipts.map((entry, i) => {
              ...
              return (
                <li key={`${c.contractHash}-${i}`}>
                  <button
                    onClick={() => setActive(i)}
                    className={`w-full text-left rounded-[6px] border p-4 transition-all ${
                      i === active ? 'bg-white/[0.06] border-stud-500/40' : 'bg-white/[0.02] border-white/10 hover:border-white/20'
                    }`}
                  >
                    ...
                    <div className="flex items-center justify-between mt-2 text-xs">
                      <span className="font-mono text-plate-300">{formatTime(c.observedAt)}</span>
                      <span className="font-mono font-bold text-plate-100">
                        {money(c.checkout.finalTotal.amount, c.checkout.finalTotal.currency)}
                      </span>
                    </div>
                    <div className="mt-1 font-mono text-[10px] text-plate-400">{shortHash(c.contractHash)}</div>
                  </button>
                </li>
              )
            })}
