      ...
      <div className="glass-card rounded-2xl border border-white/10 p-5">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          ...
          <button
            onClick={runSuite}
            disabled={busy}
            className="inline-flex items-center gap-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-sm font-medium px-5 py-2.5 transition-colors disabled:opacity-50"
          >
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-gray-500 mt-1 flex items-center gap-1.5">
                  <span className={`w-1.5 h-1.5 rounded-full ${m.breaks ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                  {m.blurb}
                </div>
              </button>
            )
      ...
      {run && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <Stat label="Resilience" value={`${resilience}%`} tone="emerald" icon={ShieldCheck} />
            <Stat label="Detected failures" value={`${run.detected}/${run.total}`} tone="rose" icon={Activity} />
            <Stat label="Previews verified" value={`${run.previewVerified}`} tone="cyan" icon={Check} />
            <Stat label="Duration" value={`${run.durationMs} ms`} tone="violet" icon={Activity} />
          </div>

          <div className="glass-card rounded-2xl border border-white/10 overflow-hidden">
            <div className="px-6 py-4 border-b border-white/10 text-sm font-mono uppercase tracking-wider text-gray-400">
...
function ResultRow({ result }: { result: StressResult }) {
  const status = result.initiallyValid
    ? { text: 'Survived', cls: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30', icon: Check }
    : result.healed
      ? { text: 'Detected → Healed', cls: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30', icon: ShieldCheck }
      : result.rejected
